...
- **Teacher matching mode** (t2va): train the text-only student against the frozen FL2VA teacher conditioned on the real first/last frames — the tab automatically builds the FL2VA latent caches and dual-presentation text caches it needs, and exposes the sigma-max band protection, DC/magnitude loss shaping, and timestep-focus controls.
